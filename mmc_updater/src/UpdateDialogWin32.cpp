...
{
	SendMessage(window, WM_SETFONT, (WPARAM)GetStockObject(DEFAULT_GUI_FONT),
				MAKELPARAM(TRUE, 0));
}
...
void UpdateDialogWin32::init(int /* argc */, char ** /* argv */)
{
	...
	m_window.CreateEx(0 /* dwExStyle */, updateDialogClassName /* class name */,
					  AppInfo::name().c_str(), style, 0, 0, width, height, 0 /* parent */,
					  0 /* menu */, 0 /* reserved */);
...
void UpdateDialogWin32::updateFinished(bool showRestart)
{
	UpdateMessage *message = new UpdateMessage(UpdateMessage::UpdateFinished);
	message->showRestart = showRestart;
	SendNotifyMessage(m_window.GetHwnd(), WM_USER, reinterpret_cast<WPARAM>(message), 0);
	UpdateDialog::updateFinished(showRestart);
}

void UpdateDialogWin32::quit()
{
	PostThreadMessage(GetWindowThreadProcessId(m_window.GetHwnd(), 0 /* process ID */), WM_QUIT,
					  0, 0);
}

LRESULT WINAPI
UpdateDialogWin32::windowProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	case WM_CLOSE:
	...
	case WM_USER:
	{
		if (window == m_window.GetHwnd())
		{
			UpdateMessage *updateMessage = reinterpret_cast<UpdateMessage *>(wParam);
			switch (updateMessage->type)
			{
			case UpdateMessage::UpdateFailed:
			{
				m_hadError = true;
				std::string text = AppInfo::updateErrorMessage(updateMessage->message);
				MessageBox(m_window.GetHwnd(), text.c_str(), "Update Problem", MB_OK);
			}
			break;
			case UpdateMessage::UpdateProgress:
				m_progressBar.SetPos(updateMessage->progress);
				break;

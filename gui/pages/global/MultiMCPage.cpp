...
void MultiMCPage::on_instDirBrowseBtn_clicked()
{
    ...
    if (!cooked_dir.isEmpty() && QDir(cooked_dir).exists())
	{
        if(checkProblemticPathJava(QDir(cooked_dir))){
           QMessageBox warning;
           warning.setText(tr("You're trying to specify an instance folder which\'s path contains at least one \'!\'. "
                              "Java is known to cause problems if that is the case, your instances (probably) won't start!"));

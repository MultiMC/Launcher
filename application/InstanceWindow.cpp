    ...
    : QMainWindow(parent), m_instance(instance)
{
    ...
    auto icon = LauncherPtr->icons()->getIcon(m_instance->iconKey());
    ...
    {
        auto base64State = LauncherPtr->settings()->get("ConsoleWindowState").toByteArray();
        restoreState(QByteArray::fromBase64(base64State));
        auto base64Geometry = LauncherPtr->settings()->get("ConsoleWindowGeometry").toByteArray();
...
{
    LauncherPtr->launch(m_instance, false, nullptr);
}
...
void InstanceWindow::closeEvent(QCloseEvent *event)
{
    ...
    LauncherPtr->settings()->set("ConsoleWindowState", saveState().toBase64());
    LauncherPtr->settings()->set("ConsoleWindowGeometry", saveGeometry().toBase64());
    emit isClosing();
    event->accept();
}
...
void InstanceWindow::on_btnKillMinecraft_clicked()
{
    if(m_instance->isRunning())
    {
        LauncherPtr->kill(m_instance);
    }
    else
    {
        LauncherPtr->launch(m_instance, true, nullptr);
    }
}

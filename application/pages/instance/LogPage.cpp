...
void LogPage::on_btnPaste_clicked()
{
    ...
    m_model->append(MessageLevel::Launcher, tr("%1: Log upload triggered at: %2").arg(LAUNCHER_BUILD_NAME).arg(QDateTime::currentDateTime().toString(Qt::RFC2822Date)));
    auto url = GuiUtil::uploadPaste(m_model->toPlainText(), this);
    if(!url.isEmpty())
    {
        m_model->append(MessageLevel::Launcher, tr("%1: Log uploaded to: %2").arg(LAUNCHER_BUILD_NAME).arg(url));
    }
    else
    {
        m_model->append(MessageLevel::Error, tr("%1: Log upload failed!").arg(LAUNCHER_BUILD_NAME));
    }
}
